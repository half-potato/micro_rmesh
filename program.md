...
4. Create `results.tsv` (header only). Verify it's in `.gitignore`.
...
- **Refinement** (`refine_bad_tets`): Fixes sliver tets by inserting at circumcenters. Maintains triangulation quality for correct sorting. → Delaunay.
