...
def query(tq, t, y, left=None, right=None):
  ...
  (i0, i1), ((yq, _),) = jax_math.sorted_lookup(tq, t, (y,), device_is_tpu())
  # Apply boundary conditions.
  left = y[Ellipsis, :1] if left is None else left
  right = y[Ellipsis, -1:] if right is None else right
  yq = jax_math.select([(i1 == 0, left), (i0 == y.shape[-1], right)], yq)
  return yq
...
def weight_to_pdf(t, w):
  ...
  td = np.diff(t)
  return np.where(td < np.finfo(np.float32).tiny, 0, jax_math.safe_div(w, td))
...
def invert_cdf(u, t, w_logits):
  ...
  t_new = jax_math.sorted_interp(u, cw, t, device_is_tpu())
  return t_new
...
def sample(
  ...
  if rng is None:
    ...
    u = np.linspace(0, 1 - u_max, num_samples) + np.random.uniform(
        low=0,
        high=max_jitter,
        size=t.shape[:-1] + (d,)
    )
...
def sample_intervals(
  ...
  samples = np.concatenate([first, mid, last], axis=-1)

  # We clamp to the limits of the input domain, provided by the caller.
  samples = np.clip(samples, *domain)
  return samples
...
def lossfun_distortion(t, w):
  ...
  loss_intra = np.sum(w**2 * np.diff(t), axis=-1) / 3
...
def weighted_percentile(t, w, ps):
  ...
  wprctile = np.vectorize(np.interp, signature='(n),(m),(m)->(n)')(
      np.array(ps) / 100, cw, t
  )
  return wprctile
...
def resample(t, tp, vp, use_avg=False):
  ...
  Notation roughly matches np.interp. Resamples by summation by default.
  ...
  if use_avg:
    ...
    v = jax_math.safe_div(v_numer, v_denom)
    return v

  acc = np.cumsum(vp, axis=-1)
  acc0 = np.concatenate([np.zeros(acc.shape[:-1] + (1,)), acc], axis=-1)
  acc0_resampled = np.vectorize(np.interp, signature='(n),(m),(m)->(n)')(
      t, tp, acc0
  )
